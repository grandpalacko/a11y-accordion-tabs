...
(function () {
    ...
    function AccordionTabs(el, options) {
        ...
        this.el = el;
        this.tabTriggers = this.el.querySelectorAll(
            '[role="tablist"][data-tabs-group="' +
                this.el.dataset.tabsGroup +
                '"] .js-tabs-trigger'
        );
        this.tabPanels = this.el.querySelectorAll(
            '[data-tabs-group="' +
                this.el.dataset.tabsGroup +
                '"].js-tabs-panel'
        );
        this.accordionTriggers = this.el.querySelectorAll(
            '[data-tabs-group="' +
                this.el.dataset.tabsGroup +
        ...
        if (el.getAttribute("data-breakpoint")) {
            this.options.breakpoint = parseInt(
                el.getAttribute("data-breakpoint")
            );
        }

        if (el.getAttribute("data-selected-tab")) {
            this.options.selectedTab = parseInt(
                el.getAttribute("data-selected-tab")
            );
        }
    ...
    AccordionTabs.prototype._clickEvent = function (e) {
        ...
        if (closestTrigger == null) {
            closestTrigger = this._getClosest(
                e.target,
                ".js-accordion-trigger"
            );
    ...
    AccordionTabs.prototype.selectTab = function (index, userInvoked) {
        ...
        if (
            !this.tabPanels[index].classList.contains("is-hidden") &&
            userInvoked
        ) {
